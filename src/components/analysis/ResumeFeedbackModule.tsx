...
const ResumeFeedbackModule: React.FC = () => {
    ...
    const fetchResumeFeedback = async () => {
        setIsLoading(true);
        try {
            const result = await apiService.getResumeFeedback();
            if (result.status === "success" && result.data) {
                const { format_score, parsing_score, feedback_points, improvement_suggestions } = result.data;

                // Type checking and validation
                if (typeof format_score === 'number' &&
                    typeof parsing_score === 'number' &&
                    Array.isArray(feedback_points) &&
                    Array.isArray(improvement_suggestions)) {
                    setFeedbackData({
                        format_score,
                        parsing_score,
                        feedback_points,
                ...
                toast.error(result.message || "Failed to fetch resume feedback");
            }
        } catch (error) {
                ...
                <div className="mt-6 space-y-6 animate-fade-in">
                    ...
                    <div className="bg-white bg-opacity-70 p-4 rounded-lg">
                        ...
                        <ul className="space-y-3">
                            {feedbackPoints.map((point: FeedbackPoint, index: number) => (
